...
D-4 가 남겨 둔 시험 기록 2개도 같은 날 정리했다(E-3). 재등록본과 "미수등"은 관리자 계정이 1분 간격으로 만든 등록 테스트 흔적이었고 응시 3건은 전부 테스트 계정이었다. 사용자 결정에 따라 원본 8문항의 해설을 재등록본 값으로 이식한 뒤 두 시험을 삭제해 `exams` 3 → 1, 배포 21 → 18, 응시 20 → 17, 보고서 10 → 9 가 됐다. 원본의 배포 18·응시 17·보고서 9 와 채점 필드 지문은 그대로다. 백업 JSON 은 학생 답안·보고서 HTML 을 담고 있어 `.gitignore` 로 추적에서 제외했다. 자세한 경과는 분석서 §10.1.4 를 참조한다.

죽은 엔드포인트 12개 삭제 여부도 2026-09-07 에 종결했다. 대체 경로가 있는 A 7개와 `/api/branch-students` 이중 마운트는 제거했고(`19443c0`, 라우트 61 → 54), 학부모 API 3개(B)와 표준 단건 조회 2개(C)는 각각 유일 경로·표준 경로라 의도적으로 유지했다(분석서 §10.3).
