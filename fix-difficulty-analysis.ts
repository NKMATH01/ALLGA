...
async function fixDifficultyAnalysis() {
  try {
    console.log('🔍 시험 조회...');
...
- 간접적이고 절제된 감정 표현`
    ...
    // Update questions with enhanced explanation
    const updatedQuestions = questionsData.map((q: any) => {
      const enhancement = difficultyEnhancements[q.number];
      if (enhancement && q.difficulty === '상') {
        return {
          ...q,
          explanation: q.explanation + enhancement,
          commentary: q.explanation + enhancement  // Add commentary field too
        };
      }
      return {
        ...q,
        commentary: q.explanation  // Ensure commentary field exists
      };
    });
    ...
    await db
      ...
      .set({
        ...
        overallReview: `## 상/최상 난이도 문항 출제 경향 - 추론과 비판적 사고 요구

### 복합 정보의 해석 요구 (7번, 20번, 24번, 30번)
복합 정보의 해석을 요구하는 문항들로, 본문 내용뿐만 아니라 <보기>에 제시된 추가 자료(그래프, 통계, 우화, 그림 등)를 정확히 해석하고 이를 본문의 정보와 결합하여 새로운 결론을 도출해야 하는 문제들이 어렵게 출제되었습니다. 단순히 내용을 찾는 것을 넘어, 정보를 가공하고 적용하는 능력을 요구합니다.

### 문법의 예외적 규정 (12번, 14번)
문법의 기본 개념을 넘어, 특수한 조건에서만 성립하는 예외적인 규정이나 용법을 정확히 알고 있는지를 물었습니다. ('안절부절못하다'처럼 부정어와만 결합하는 용언, '-었-'의 미래 표현 용법 등) 암기만으로는 부족하며, 개념에 대한 깊이 있는 이해가 필요합니다.

### 외적 준거를 활용한 감상 (33번, 36번)
<보기>에 제시된 시대적 배경, 사회 비평적 관점, 작가 정보 등 외적 준거를 바탕으로 작품을 깊이 있게 감상하고 비판하는 능력을 요구했습니다. 작품 내적인 요소뿐만 아니라 외부 정보와의 연결고리를 찾아 해석해야 하므로 종합적인 사고력이 필요합니다.`
      })
      .where(eq(exams.id, exam.id));

    console.log('✅ 난이도 분석 및 추가 해설 수정 완료!');
    console.log(`업데이트된 문항: ${Object.keys(difficultyEnhancements).join(', ')}번`);
    ...
    const updatedQuestionsData = updatedExam.questionsData as any[];
    const question7 = updatedQuestionsData.find((q: any) => q.number === 7);

    console.log('\n7번 문항 확인:');
    console.log(`난이도: ${question7.difficulty}`);
    console.log(`commentary 존재: ${!!question7.commentary}`);
    console.log(`commentary 길이: ${question7.commentary?.length || 0}자`);
